...
var s_account="forbesinvest"
//var s_account="forbesinvestdev"
var s_dynamicAccountSelection=false
var s_dynamicAccountList="report_suite1=word1;report_suite2=word2"
...
var s_linkDownloadFileTypes="exe,zip,wav,mp3,mp4,mov,mpg,avi,wmv,doc,pdf,xls"
var s_linkInternalFilters="javascript:,investopedia.com"
...
function s_doPlugins() {
  ...
  // TODO::RETAIN DOMAIN NAME IN PAGE NAME
  if ((!window.s_pageType) && (!window.s_pageName || s_pageName==""))
    s_vpr('s_pageName', s_vp_getPageName());
  // TODO::SET CHANNEL & SECTION VALUES
  //s_vpr('s_channel', (window.channel+':'+window.section));
  // TODO::ADD PARTNER REFERRALS & DEPTH TRACKING
  s_vp_getCGI('s_prop1','partner');
  s_vpr('s_prop2',s_getPartnerName());
  // PROP3 IS A COUNT OF PAGEVIEWS WITH UID COOKIE
  investUID = document.cookie.indexOf('UID=');
  if (investUID != -1)
    s_vpr('s_prop3','UID');
  // PROPS 4-6 TRACK URL PARAMS FOR SEARCHES, SYMBOLS & TAGS
  s_vp_getMultiCGI('s_prop4','q',':');
  s_vp_getMultiCGI('s_prop5','s',':');
  s_vp_getMultiCGI('s_prop6','tag',':');

}
/************************** PLUGINS SECTION *************************/
/* You may insert any plugins you wish to use here.                 */
function s_getPartnerName()
{
	var pName="";
	var partnerName = s_c_r("s_p_name");
	if (partnerName==null||partnerName.length<=0)
	{
		s_vp_getCGI('tmpVar','partner');
		pName=s_vp_getValue('tmpVar')
		if (pName&&pName.length>0) s_c_w("s_p_name",pName);
	} 
	else
		pName=partnerName;
	return pName;
...
/*
 * Plugin: Version 1.0 - Populate a variable with the concatenated
 * value of multiple query string parameters.
 */
...
/* Remote Code - Dynamic - G.9 patch 2 */
...
im,b,e,rs='http'+(s_ssl?'s':'')+'://'+(s_ssl?'sc-investopedia':'sc-investopedia')+'.investopedia.com/b/ss/'+un+'/1/G.9p2/'
