...
function addHandler(element, eventName, eventHandler, owner)
{
  ...
  var func 
  if (owner)
    func = function() { owner[eventHandler](element, owner); };
  else
    func = eventHandler;
...
{
  var key = getAttributeValue(element, timerKey);
  if (key != '')
    clearTimeout(key);
    
  key = setTimeout(method, delay);
...
function asyncMethodCall(method, delay, owner, timerKey)
{
  ...
  {
    var timer = owner.getTimer(timerKey);
    if (timer != '')
      clearTimeout(key);
...
function beginAsync(object, method, delay, timer)
{
  ...
  var func = function() { method.apply(object, parameters);};
...
{
  var boxes = selectedItems_getControls(collection);
  
  for (k = 0; k < boxes.length; k++)
  {
    addHandler(boxes[k], 'onclick', selectedItemsHyperLink_onCheckChanged);
...
function selectedItemsHyperLink_onCheckChanged(e)
{
  ...
  var collection = getAttributeValue(sender, 'collection');
...
{
  var offset = getLeft(target);
  offset += getWidth(target);
  offset -= getWidth(element);
  return offset;
}
...
{ 
  var value = getAttributeValue(sender, 'value'); 
  var selected = selectedItems_isSelected('SelectedPrimitives', value); 
  if (selected) 
  { 
    selectedItems_setSelected('SelectedPrimitives', value, false); 
    var text = getAttributeValue(sender, 'unselectedText'); 
  } 
  else 
  { 
    selectedItems_setSelected('SelectedPrimitives', value, true); 
    var text = getAttributeValue(sender, 'selectedText'); 
  } 
  setInnerText(sender.childNodes[0], text); 
...
{
  var code = getAttributeValue(sender, 'value');

  if (!MetaPress.Products.Reader.Web.UI.PrimitiveHeadingControl.ToggleTocAlert(code).value)
    return true;
    
  var eText = getAttributeValue(sender, 'enabledText');
  var dText = getAttributeValue(sender, 'disabledText');
  
  if (getInnerText(sender) == eText)
    setInnerText(sender, dText);
  else
    setInnerText(sender, eText); 
...
{  
  ...
  sender.style.left = 1000000; // horrible horrible hack!
}
